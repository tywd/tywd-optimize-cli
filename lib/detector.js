// 检查当前环境并动态导入所需模块
let fs, path, glob, ejs, child_process;

// 使用动态导入以支持ES Module和CommonJS
const initModules = async () => {
  if (typeof require !== 'undefined') {
    ...
    ejs = ejsModule.default || ejsModule;
      ...
      categoryResults.forEach(result => {
        if (result.status === 'passed') {
          ...
          if (result.recommendation) {
            ...
            if (result.docPath) {
              console.log(`     📚 详细文档: https://tywd.github.io/optimize-cli/${result.docPath}.html`);
            }
          }
        }
      });
...
<body>
    ...
    // 在浏览器中打开报告
    child_process.exec(`open ${reportPath}`, (error) => {
      if (error) {
...
// 兼容ES Module和CommonJS导出
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { OptimizerDetector };
} else if (typeof exports !== 'undefined') {
  exports.OptimizerDetector = OptimizerDetector;
}